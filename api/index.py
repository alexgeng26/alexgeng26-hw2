...
def base64_to_number(b64_str):
    ...
    try:
        ...
        # Always use signed=True since we always encode with signed=True
...
def number_to_base64(number):
    ...
    try:
        ...
        # Always use signed=True to handle both positive and negative numbers
        if number.bit_length() == 0:
            byte_count = 1
        else:
            # For signed integers, we need one extra bit for the sign
            byte_count = (number.bit_length() + 8) // 8
